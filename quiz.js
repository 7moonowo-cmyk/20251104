...
function setup(){
  ...
  loader('questions.csv').then(qs => {
    questions = qs || [];
    // 如果共用 mapRowsToQuestions 已套，questions 為 question 物件陣列；若不是（fallback），嘗試做基本檢查
    if (questions.length && questions[0].question === undefined && typeof mapRowsToQuestions === 'function') {
      // 若 loader 回傳 raw rows，嘗試轉換
...
// 若滑鼠點擊選單，交給共用處理
function mousePressed(){ if (isMenuActive(mouseX, mouseY)) handleCommonMenuPressed(mouseX, mouseY); }